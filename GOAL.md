...
> **Hackathon deadline:** Aug 23, 11:45 PM PT (14 days out)
> **Devpost:** https://neuralsprint.devpost.com/
> **Prize pool (as listed):** $150
> **Final submit step:** https://devpost.com/submit-to/30481-neuralsprint/manage/submissions/1131630/finalization
...
### 0A.1 This repository's exclusive port block: `4210`–`4219`

| Port | Service |
|---|---|
| `4210` | Vite local-first app dev server |
| `4211` | Preview server (must pass with the network fully disabled) |
| `4212` | Playwright webServer |
| `4213` | Adversarial fixture server (serves generated attack PDFs) |

Unassigned ports in this block (`4214`, `4215`, `4216`, `4217`, `4218`, `4219`) are reserved for services discovered later. **Never allocate a port outside `4210`–`4219`.**
...
Ports are not the only shared namespace. Prefix every one of these with this repository's directory name, `neuralsprint`:

- `COMPOSE_PROJECT_NAME=neuralsprint` on every Docker Compose invocation, with container names prefixed to match.
- Database name and role: `neuralsprint`.
- Redis logical database index: **11** (this repository's index in the sibling set of sixteen).
- Object-storage bucket or MinIO alias: `neuralsprint`.
...
```sh
# neuralsprint — exclusive block 4210-4219
PORT_0=4210   # Vite local-first app dev server
PORT_1=4211   # Preview server
PORT_2=4212   # Playwright webServer
PORT_3=4213   # Adversarial fixture server
```
...
A local-first forensic redaction verifier and repair tool: prove whether covered text, OCR layers, annotations, form fields, metadata, attachments or earlier incremental-save revisions still expose supposedly redacted content, rebuild a genuinely sanitized copy, then run the same attacks against its own output before declaring anything safe.

> It attacks its own output before it is willing to show you a green result, and it names exactly which attacks it ran.
...
1. A deployed local-first application that provably functions with the network disabled after load.
2. A real-world PDF corpus trial including documents produced by at least five distinct authoring tools.
3. A versioned, published threat model and support matrix stating exactly what the tool does not cover.
4. A signed release with a reproducible build so users can verify the bytes they run.
...
1. Implement the parser for xref tables AND xref streams, the full object graph, filter chains and the complete incremental revision chain.
2. Build the content-stream interpreter tracking paint order, transforms, clipping and glyph geometry, so text hidden under an opaque rectangle or image is detectable geometrically rather than heuristically.
3. Enumerate and type every leak class: covered text, OCR layer, annotations, form fields, XMP and Info metadata, embedded files and attachments, earlier revisions, JavaScript, optional content groups.
4. Implement the sanitizer as a fresh object graph rebuild with an allowlist, plus a raster fallback for irreducible regions; incremental append is prohibited by construction.
5. Build the independent verifier that reloads the EMITTED BYTES in a fresh worker with no shared state and runs structural, text-extraction, byte-grep, OCR and pixel-difference checks.
6. Mask real secrets everywhere by default: UI, logs, telemetry, screenshots and exported reports, using structural masking rather than string replacement.
7. Build the adversarial fixture generator producing every attack class with seeded tokens and machine-checkable expected results.
8. Make NOT VERIFIED the default outcome for any unknown parser, filter or structure state, and make green results enumerate exactly which attacks passed.
9. Handle large files with bounded memory, cancellation and recovery, verified against multi-hundred-megabyte documents.
10. Version the threat model, support matrix and report schema so a report from six months ago is still interpretable.
...
1. A visual overlay is never treated as removal, no matter how opaque, and the code has no path that could conclude otherwise.
2. An unsupported filter, malformed xref or unparseable revision chain yields NOT VERIFIED, never a qualified green.
3. A verification pass that cannot complete is a failure, not a skip.
4. A green result never implies universal safety; it names its scope explicitly, and copy asserting otherwise fails a build lint.
...
**1. `src/pdf/parser` — xref tables/streams, object graph, filters, revision chain**
...
**2. `src/pdf/interpreter` — paint order, transforms, glyph geometry, hidden content**
   - Build the complete production surface of this area, not a representative subset.
   - Encode this area's share of the domain invariants as types, schema constraints, or boundary assertions.
   - Implement every designed failure, refusal, partial, stale and cancelled state this area can enter.
   - Add structured logging, metrics and traces with stable event names and correlation IDs.
...
**3. `src/findings` — typed leak classes and masked evidence**
   - Build the complete production surface of this area, not a representative subset.
   - Encode this area's share of the domain invariants as types, schema constraints, or boundary assertions.
   - Implement every designed failure, refusal, partial, stale and cancelled state this area can enter.
   - Add structured logging, metrics and traces with stable event names and correlation IDs.
...
**4. `src/sanitizer` — vector removal, raster fallback, allowlisted rebuild**
   - Build the complete production surface of this area, not a representative subset.
   - Encode this area's share of the domain invariants as types, schema constraints, or boundary assertions.
   - Implement every designed failure, refusal, partial, stale and cancelled state this area can enter.
   - Add structured logging, metrics and traces with stable event names and correlation IDs.
...
**5. `src/verifier` — fresh-process structural, textual, byte, OCR, and pixel checks**
   - Build the complete production surface of this area, not a representative subset.
   - Encode this area's share of the domain invariants as types, schema constraints, or boundary assertions.
   - Implement every designed failure, refusal, partial, stale and cancelled state this area can enter.
   - Add structured logging, metrics and traces with stable event names and correlation IDs.
...
**6. `src/ui` — page review, region editing, status, accessibility**
   - Build the complete production surface of this area, not a representative subset.
   - Encode this area's share of the domain invariants as types, schema constraints, or boundary assertions.
   - Implement every designed failure, refusal, partial, stale and cancelled state this area can enter.
   - Add structured logging, metrics and traces with stable event names and correlation IDs.
...
**7. `fixtures` — generated adversarial PDFs and expected results**
   - Build the complete production surface of this area, not a representative subset.
   - Encode this area's share of the domain invariants as types, schema constraints, or boundary assertions.
   - Implement every designed failure, refusal, partial, stale and cancelled state this area can enter.
   - Add structured logging, metrics and traces with stable event names and correlation IDs.
...
1. Every required fixture class detected and repaired as expected, with the fixture corpus regenerated deterministically from seeds.
2. Seeded-token residual count across decoded streams, text extraction, OCR, metadata, attachments and revisions, which must be exactly zero.
3. False-green rate on unsupported and malformed inputs, which must be exactly zero.
4. No-network and document-privacy tests covering every code path including error and cancellation paths.
5. Memory, cancellation and recovery budgets measured against a large-document corpus.
...
**G1. All required fixture classes detect and repair as expected**
...
**G2. No seeded token remains in decoded streams, extraction, OCR, metadata, attachments, or revisions**
   - Epoch 1: satisfy it and commit the regenerating command plus its output.
   - Epoch 2: widen the input space the gate is evaluated over, then satisfy it again.
   - Epoch 3: tighten the numeric threshold and satisfy it again.
   - Epoch 4+: attack the gate adversarially, find the input class that breaks it, then satisfy it again on the widened class. A gate that has never failed has never been tested.

**G3. Unsupported/malformed cases never show green**
   - Epoch 1: satisfy it and commit the regenerating command plus its output.
   - Epoch 2: widen the input space the gate is evaluated over, then satisfy it again.
   - Epoch 3: tighten the numeric threshold and satisfy it again.
   - Epoch 4+: attack the gate adversarially, find the input class that breaks it, then satisfy it again on the widened class. A gate that has never failed has never been tested.

**G4. No-network/document privacy tests pass**
   - Epoch 1: satisfy it and commit the regenerating command plus its output.
   - Epoch 2: widen the input space the gate is evaluated over, then satisfy it again.
   - Epoch 3: tighten the numeric threshold and satisfy it again.
   - Epoch 4+: attack the gate adversarially, find the input class that breaks it, then satisfy it again on the widened class. A gate that has never failed has never been tested.

**G5. Large-file memory/cancel/recovery budgets pass**
   - Epoch 1: satisfy it and commit the regenerating command plus its output.
   - Epoch 2: widen the input space the gate is evaluated over, then satisfy it again.
   - Epoch 3: tighten the numeric threshold and satisfy it again.
   - Epoch 4+: attack the gate adversarially, find the input class that breaks it, then satisfy it again on the widened class. A gate that has never failed has never been tested.

**G6. Threat model, support matrix, and report schema are versioned**
   - Epoch 1: satisfy it and commit the regenerating command plus its output.
   - Epoch 2: widen the input space the gate is evaluated over, then satisfy it again.
   - Epoch 3: tighten the numeric threshold and satisfy it again.
   - Epoch 4+: attack the gate adversarially, find the input class that breaks it, then satisfy it again on the widened class. A gate that has never failed has never been tested.

**Submission mechanics** (deadline Aug 23, 11:45 PM PT (14 days out)):
...
|---|---|---|---|---|
| Leak classes covered with fixtures | 6 | 10 | 16 | 24 |
| Adversarial fixtures generated | 50 | 300 | 1500 | 6000 |
| Seeded-token residuals after sanitization | 0 | 0 | 0 | 0 (against 10x more attacks) |
| False-green rate on malformed inputs | 0 | 0 | 0 | 0 (under structured fuzzing) |
| Authoring tools represented in the real corpus | 3 | 8 | 15 | 30 |
| Largest document handled within memory budget | 50MB | 200MB | 600MB | 2GB |
| PDF filters supported or explicitly refused | 6 | 12 | 20 | all standard |
| Mutation score on parser, sanitizer and verifier | 65% | 80% | 90% | 95% |
...
1. A black rectangle drawn over text that remains fully extractable.
2. An OCR layer added after redaction that re-encodes the covered text.
3. A redaction applied then undone in a later incremental revision.
4. A form field whose default value contains the secret.
5. An embedded attachment containing the original unredacted document.
6. A font subset whose glyph names leak the covered characters.
7. Text hidden by a clipping path rather than an overlay.
8. A scanned image where the secret survives only as pixels and needs OCR to find.
9. An optional content group hiding a layer that a different viewer will show.
10. A four-hundred-megabyte document that exhausts memory during rebuild.
11. A malformed xref that parses differently in two libraries.
12. A user screenshotting a report that accidentally rendered a real secret.
